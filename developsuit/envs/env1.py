...
class Env(MultiRobotsObsAvoid):
    def __init__(self, n_robots, show_mode="no_show"):
        super().__init__(n_robots=n_robots, show_mode=show_mode, obstacle_have=True, obstacle_size_mode="random",
                         obstacle_pos_mode="fixed", fix_obs_arena_name="Fix_Obs_paper")
        self.target_board = None
        self.n_robots = n_robots
        self.l_form = 0.75
        self.range_cmftble = 0.15
    ...
    def reset(self):
        ...
        self.physics.bind(self.mocap_t).mocap_pos[:] = np.array([self.target_board[0], self.target_board[1], 0.5])
