...
def quat_distance(quaternion1, quaternion0):
    ...
    """
    return quat_norm(quat_multiply(quaternion1, quat_inverse(quaternion0)))


def quat_norm(quat):
    quat = quat / np.linalg.norm(quat.reshape([-1]))
    return quat
...
def quat2eul(quat, mode="ZYX"):
    ...
    if mode == "ZYX":
        ...
        if sing_val > threshold:
            ...
            rx = 0
            ry = math.pi / 2
            rz = -2 * math.atan2(q1, q0)
        elif sing_val < -threshold:
            rx = 0
            ry = -math.pi / 2
            rz = 2 * math.atan2(q1, q0)
        else:
            rx = math.atan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 ** 2 + q2 ** 2))
            ry = math.asin(2 * (q0 * q2 - q1 * q3))
            rz = math.atan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2 ** 2 + q3 ** 2))
    elif mode == "XZY":
        rx = math.atan2(2 * (q2 * q3 + q0 * q1), 1 - 2 * (q1 ** 2 + q3 ** 2))
        ry = math.atan2(2 * (q1 * q3 + q0 * q2), 1 - 2 * (q2 ** 2 + q3 ** 2))
        rz = math.asin(2 * (q0 * q3 - q1 * q2))
    else:
        raise ValueError("quat2eul 给入未写欧拉角顺序，请对程序进行补充")

    eul = np.array([rx, ry, rz]).reshape([3, 1])
