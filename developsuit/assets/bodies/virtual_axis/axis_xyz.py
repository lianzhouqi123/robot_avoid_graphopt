...
class Axis_xyz:
    def __init__(self, arena):
        ...
        self.axis_y = Primitive(name="axis_y", type="sphere", size="0.01", rgba="0 0 0 0", conaffinity=0,
                                contype=0, option="geom")
        self.axis_z = Primitive(name="axis_z", type="sphere", size="0.01", rgba="0 0 0 0", conaffinity=0,
                                contype=0, option="geom")
        ...
        frame_axis_z = self.axis_y.mjcf_model.attach(self.axis_z.mjcf_model)
        joint_z = frame_axis_z.add('joint', type='slide', axis=[0, 0, 1])
        self.joints = [joint_x, joint_y, joint_z]

    def attach(self, child):
        frame_child = self.axis_z.mjcf_model.attach(child.mjcf_model)
        return frame_child

    @property
    def geom(self):
        return [self.axis_x.geom, self.axis_y.geom, self.axis_z.geom]
