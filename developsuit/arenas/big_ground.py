...
class Arena:  # 场景初始化
    def __init__(self) -> None:
        ...
        self.size_ground = np.array([3.25, 5.5])
        size_block = 0.5
        ...
        grid = self._mjcf_model.asset.add(
            ...
            name="grid",
            texture=chequered,
            texrepeat=[self.size_ground[0]*2/size_block, self.size_ground[1]*2/size_block],
            reflectance=0.001,
        )
        self.ground_geom = self._mjcf_model.worldbody.add(
            "geom", type="plane", size=[self.size_ground[0], self.size_ground[1], 0.01], material=grid, contype=1, conaffinity=1)
